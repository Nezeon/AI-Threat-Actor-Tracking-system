...
const ThreatActorPanel: React.FC = () => {
  ...
  useEffect(() => {
    getAllActors()
      .then((serverActors) => {
        if (serverActors.length > 0) {
          setActors(serverActors);
          localStorage.setItem('hivepro_threat_actors', JSON.stringify(serverActors));
        }
      })
      .catch((err) => {
        console.warn("Could not fetch actors from server, using local cache:", err.message);
      });
  }, []);

  // Persist actors to localStorage cache
  useEffect(() => {
  ...
  const handleSectionRefresh = async (section: 'ALIASES' | 'DESCRIPTION' | 'CVES') => {
    ...
    try {
      const partialData = await refreshActorSection(selectedActor.id, selectedActor.name, section);
  ...
  const handleDeleteActor = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
